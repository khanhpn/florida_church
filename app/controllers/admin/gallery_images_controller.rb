class Admin::GalleryImagesController < Admin::AdminController
  ...
  def create
    ...
    if gallery_image.save
      ...
      flash.now[:warning] = 'You can not create gallery_image, please check error'
      render :new
    end
  end
  ...
  def update
    if @gallery_image.update(params_gallery_image)
      # gallery_image.contents.attach(params[:gallery_image][:contents])
  ...
  def params_gallery_image
    params.require(:gallery_image).permit(:name, :display_order, :display, contents: [])
  end
